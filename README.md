...
The tools expose persisted event pages, not a full-text index or retained-episode body view. `contains` is an exact case-sensitive literal scan, not lexical/BM25 ranking. Long event payloads return bounded excerpts with `payload_start_char` and `payload_truncated=true`; filtered results center the match when possible, while exact full-payload expansion remains a separate follow-up capability. A `has_more=false` response is exhaustive for the selected stream and literal filter, but cannot recover data NAC never persisted.
