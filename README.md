...
Before each ordinary model call, a session-backed orchestrator whose estimated context reaches the threshold can replace older provider-visible history with a durable historical summary while retaining the two most recent user turns. The complete canonical transcript remains unchanged in session storage, checkpoint rows stay private, and workers never compact. The threshold is a proactive trigger rather than a hard context limit; an oversized retained tail can still produce a terminal `finish_reason=length`. Existing valid checkpoints remain active on resume even when creating new checkpoints is disabled.
