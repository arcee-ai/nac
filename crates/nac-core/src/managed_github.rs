...
use crate::model::auth_store::{
    read_auth_string_from_path, remove_auth_file_from_path, try_acquire_credential_lock,
...
impl ManagedGitHubAuth {
    ...
    pub fn disconnect(&self) -> Result<bool> {
        crate::model::auth_store::with_credential_lock(&self.lock_path, || {
    ...
    pub async fn current_token(&self) -> Result<Option<GitHubAccessToken>> {
        loop {
            let Some(_lock) = try_acquire_credential_lock(&self.lock_path)? else {
    ...
    async fn complete_device_login(
        ...
        loop {
            ...
            crate::model::auth_store::with_credential_lock(&self.lock_path, || self.save(&stored))?;
            return self.status();
        }
    }
    ...
    fn force_access_expiry(&self) -> Result<()> {
        crate::model::auth_store::with_credential_lock(&self.lock_path, || {
            if let Some(mut auth) = self.load()? {
    ...
    fn load(&self) -> Result<Option<StoredGitHubAuth>> {
        ...
        if stored.access_token.is_empty()
            ...
            || !stored.organization.eq_ignore_ascii_case(ORGANIZATION)
        {
    ...
    fn save(&self, stored: &StoredGitHubAuth) -> Result<()> {
        write_auth_string_to_path(
            &self.path,
...
mod tests {
    ...
    async fn concurrent_refresh_rotates_once_and_revocation_removes_local_auth() {
        let (endpoints, requests, server) =
            scripted_server(vec![json_response(serde_json::json!({
        ...
        let auth = ManagedGitHubAuth::with_endpoints(&root, "Iv1.test", endpoints).unwrap();
        auth.save(&stored_auth("stale-access", "old-refresh", 0))
            .unwrap();
        ...
        let stored = auth.load().unwrap().unwrap();
        ...
        let revoked =
            ManagedGitHubAuth::with_endpoints(&root, "Iv1.test", revoked_endpoints).unwrap();
