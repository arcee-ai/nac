...
pub(crate) struct NewOrchestratorCompactionCheckpoint {
    ...
    /// Digest of `messages[tail_start_message_index..]` at checkpoint
    /// creation; NULL on legacy rows.
    pub tail_sha256: Option<[u8; 32]>,
...
const CHECKPOINT_COLUMNS: &str =
    "id, session_id, previous_checkpoint_id, summary, tail_start_message_index, \
     tail_message_count, tail_sha256, source_prefix_sha256, system_policy_sha256, \
...
pub(crate) fn append_orchestrator_compaction_checkpoint(
    ...
    transaction.execute(
        ...
        params![
            ...
            tail_start_message_index,
            tail_message_count,
            checkpoint.tail_sha256.as_ref().map(<[u8; 32]>::as_slice),
...
fn map_checkpoint_row(
    ...
    Ok(OrchestratorCompactionCheckpoint {
        ...
        tail_sha256: row
            ...
            .map(|value| digest_from_sqlite(value, 6))
            .transpose()?,
        source_prefix_sha256: digest_from_sqlite(row.get(7)?, 7)?,
        system_policy_sha256: digest_from_sqlite(row.get(8)?, 8)?,
        prompt_policy_version: u32_from_sqlite(row.get(9)?, 9)?,
        old_context_estimate: token_count_from_sqlite(row.get(10)?, 10)?,
        ...
        summary_completion_tokens: row
            .get::<_, Option<i64>>(12)?
            .map(|value| token_count_from_sqlite(value, 12))
            .transpose()?,
        new_context_estimate: token_count_from_sqlite(row.get(13)?, 13)?,
...
mod tests {
    ...
    fn append_round_trips_exact_values_and_loads_newest_first() {
        ...
        assert_eq!(second.tail_sha256, Some([7; 32]));
