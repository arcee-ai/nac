...
fn delete_session_uses_cascades_for_owned_auxiliary_rows() {
    ...
    assert_eq!(
        crate::store::load_all_thread_events(&store_path, "session-del", 20).unwrap()["auth"].len(),
        1
    );
...
fn presentation_titles_are_normalized_validated_and_clearable() {
    ...
    let titled =
        update_session_presentation(&store_path, "session-title", "  Custom  title  ", false, 0)
            .unwrap();
    assert_eq!(titled.title.as_deref(), Some("Custom  title"));
    assert_eq!(titled.presentation_version, 1);

    let cleared =
        update_session_presentation(&store_path, "session-title", " \u{2003} ", false, 1).unwrap();
    ...
    let no_op = update_session_presentation(&store_path, "session-title", &unicode_title, false, 3)
        .unwrap();
...
fn run_state_loader_does_not_deserialize_transcript_messages() {
    ...
    let (run_state, updated_at) = load_session_run_state(&store_path, "session-run-state").unwrap();
