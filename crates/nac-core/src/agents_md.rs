...
impl AgentsMdBundle {
    pub fn load(workspace_dir: Option<&Path>, paths: &PathContext) -> Result<Self> {
        let settings = load_settings(paths);
        let mut files = Vec::new();

        if let Some(global_file) = select_non_empty_file(
            paths.nac_home_dir().as_deref(),
...
fn load_settings(paths: &PathContext) -> AgentsMdSettings {
...
mod tests {
    ...
    fn non_git_scope_only_loads_current_directory_file() {
        ...
        let bundle = AgentsMdBundle::load(Some(&child), &PathContext::new(&child)).unwrap();
        let contents: Vec<&str> = bundle
            .files()
            .iter()
            .map(|file| file.content.as_str())
    ...
    #[test]
    fn relative_nac_home_uses_explicit_path_context() {
        let _guard = TEST_ENV_LOCK.lock().unwrap();
        let root = temp_dir("relative_nac_home");
        let nac_home = root.join("nac-home");
