//! Hardened private-file persistence and cross-process locking for credentials.
//!
//! This infrastructure is shared by ordinary model authentication and the
//! optional managed product without either bounded context depending on the other.
...
pub fn try_acquire_credential_lock(path: &Path) -> Result<Option<FileLock>> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .with_context(|| format!("failed to create {}", parent.display()))?;
    }
...
pub fn acquire_credential_lock(path: &Path) -> Result<FileLock> {
    acquire_lock(path)
}

pub fn read_auth_string_from_path(path: &Path) -> Result<Option<String>> {
...
#[derive(Debug)]
pub struct UnsafeCredentialPermissionsError {
    path: PathBuf,
    mode: u32,
}
...
pub fn write_auth_string_to_path(path: &Path, raw: &str) -> Result<()> {
    atomic_replace_auth_file(path, |file| file.write_all(raw.as_bytes()))
}

pub fn remove_auth_file_from_path(path: &Path) -> Result<bool> {
...
pub fn ensure_open_credential_file_is_safe(file: &File, path: &Path) -> Result<()> {
...
#[cfg(test)]
fn assert_lock_contention_and_release(
    label: &str,
