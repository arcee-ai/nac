...
enum RootCommand {
    ...
    #[command(version, long_version = BUILD_VERSION)]
...
"#,
    ...
    fn upgrade_command_parses_arguments() {
        ...
        assert!(matches!(
            cli.command,
            Some(RootCommand::Upgrade(UpgradeCli { install_dir: None }))
        ));

        let cli =
            Cli::try_parse_from(["nac-web", "upgrade", "--install-dir", "/tmp/test"]).unwrap();
