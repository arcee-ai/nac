...
The catalog also records each model's `modalities.input` image capability. NAC exposes image-aware `read` behavior only when both that flag and the selected wire adapter support native image tool results; currently this is the OpenAI Responses path, including managed Codex. Supported PNG, JPEG, WebP, and static GIF results stay typed through transcript persistence and are sent as `input_image` content. Other model/adapter combinations keep the text-only `read` description and reject image files.
...
- `set` accepts `display_name`, `context_window`, `max_tokens`, `cost` (`input`/`output`/`cache_read`/`cache_write` per 1M tokens, plus an optional `tiers` list), `cache_write_1h`, `reasoning`, `image_input`, and `thinking_level_map`. The wire protocol (`api`) and adapter quirks (`compat`) are not patchable. Omitted `cost` buckets keep their resolved rates. A `cost` patch without `tiers` keeps the entry's existing tiers; an explicit `"tiers": []` clears them. Buckets a tier omits fill from the resulting merged base rates, so partial tiers stay complete rate sets.
- `image_input` overrides the models.dev input-modality flag for custom or incorrectly classified models. It does not enable images on a wire adapter that cannot represent typed image tool results.
