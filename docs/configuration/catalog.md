...
- `set` accepts `display_name`, `context_window`, `max_tokens`, `cost` (`input`/`output`/`cache_read`/`cache_write` per 1M tokens, plus an optional `tiers` list), `cache_write_1h`, `reasoning`, and `thinking_level_map`. The wire protocol (`api`) and adapter quirks (`compat`) are not patchable. A `cost` patch without `tiers` keeps the entry's existing tiers; an explicit `"tiers": []` clears them. Buckets a tier omits fill from the patch's base rates, so partial tiers stay complete rate sets.
